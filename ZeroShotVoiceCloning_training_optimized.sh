#!/usr/bin/env bash
# Zero-Shot Voice Cloning Training Script with Memory Optimization
# Usage: ./ZeroShotVoiceCloning_training_optimized.sh [fp16|bf16]
...
if torch.cuda.is_available():
    ...
    if total_memory < 12:
        print('WARNING: Low GPU memory detected. Using optimized settings.')
else:
...
echo "[INFO] Starting Memory-Optimized Zero-Shot Voice Cloning training..."

# Create output directory with timestamp
OUTPUT_DIR="/root/data/higgs/train-higgs-audio-vi/runs/zero_shot_voice_cloning_optimized_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUTPUT_DIR"
...
python3 trainer/trainer.py \
  ...
  --logging_dir "./logs/zero_shot_voice_cloning_optimized" \
